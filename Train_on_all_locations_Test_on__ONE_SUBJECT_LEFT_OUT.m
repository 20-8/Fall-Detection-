...
%% Import Code to Call Particular File with specific number or name
 ...
 s1='P18'   %% this is file that is to be compared and to be called upon

for t=1        :length(Files)
    
myPath = Files{1, t}  %% specfic file being called
C = strsplit(myPath,'A')   %% a character is given where the string is divided
CC{t,1}=C{1,1}
...
get_specific_file_indices = cell2mat(tf);  %% indices for specific file being called 

Files=Files';
 Specific_File= Files(get_specific_file_indices,:);
 
 
 %% Get Training and Testing Indices and Files
 
 training_files_indices=~(get_specific_file_indices); %% Training Indices
 
training_files=Files(training_files_indices,:);         %% Training Files

testing_files_indices=get_specific_file_indices;      %% Test Files Indices

 test_files= Files(testing_files_indices,:);             %% Test Files 

 %% Leave out the Fall Activity P08A06R1

 s1='P08A06'   %% this is file that is to be compared and to be called upon
 CC=cell(length(training_files), 1);
clear tf

for t=2        :length(training_files)
    
myPath = training_files{t, 1}  %% specfic file being called
C = strsplit(myPath,'R')   %% a character is given where the string is divided
CC{t,1}=C{1,1}


tf(t,1) = strncmp(s1,CC{t,1},6); %% string compare if it equial to s1, will save 1 otherwise 0. logical array stored. 
...
training_files= training_files(get_specific_file_indices,:);
...
z = cell(length(Files), 1);
 %%
for FileIndex=1:  length(training_files)
...
tempCounter=0
for FileIndex=1:  length(test_files)
    tempCounter=tempCounter+1;
    %% Ancortek reading part
    filename=Files(FileIndex);
    fileID = fopen(horzcat(Data_dir,char(filename)), 'r');
...
test_features1(tempCounter,:) = [mad(BanC1),rms(BanC1),skewness(BanC1),var(BanC1),mean(BanC1), std(BanC1), kurtosis(BanC1),                    mad(DopC1),rms(DopC1),skewness(DopC1),var(DopC1), mean(DopC1), std(DopC1), kurtosis(DopC1)];  %% Three dimentional matrix k is the repetion , m is the total number of movements
...
%% label leave one out files 
Grp=length(test_files)/5;
label_A_testData(1:Grp,:)={'A'};

label_A_testData((1*Grp)+1:2*Grp,:)={'B'};

label_A_testData((2*Grp)+1:3*Grp,1)={'C'};

label_A_testData((3*Grp)+1:4*Grp,1)={'D'};

label_A_testData((4*Grp)+1:5*Grp,1)={'E'};
...
prediction_test_leave1out = predict(Mdl,test_features1);         %% Preparing for  confusion matrix for training dataset

Conf_Mat_Test = confusionmat(prediction_test_leave1out,label_A_testData);      % Confusion Matrix for Test Data 
