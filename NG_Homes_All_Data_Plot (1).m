%% Save 5 activities from NG homes in numerical order and save the bandwidth and centroid 
...
BaseName='R';
for k=1     :5
number_of_activities{k}=['0',num2str(k),BaseName]        %
end


 CC=cell(length(Files), 1);
%% Activity 1 Extractions
...
tf(t,act) = strncmp(number_of_activities{1, act}  ,CC{t, 1},3);   %%% same indices for each activity namely activy 1 activity 2 activy 3 and so on up to activity 5
...
for act1=1      :5
 ...
 Activity{act1} =Files(tf(:,act1),:)   %%% Save each activity file name in corresponding cell 

end



%%% Data Processing for Ancortek
...
clear Files
 ...
 Files= [Activity{1, 1} ; Activity{1, 2}; Activity{1, 3} ;Activity{1, 4} ;  Activity{1, 5} ]  %% Save activity files in order activty 1 to 5
